...
class GrowDisc(nn.Module):
    def __init__(self, fused=True, from_rgb_activate=True):
        ...
        self.progression = nn.ModuleList(
            [
                ...
                ConvBlock(16, 32, 3, 1, downsample=True, fused=fused),  # 256
            ]
        )
    ...
    def forward(self, input, split, step=0, alpha=-1):
        for i in range(step, -1, -1):
            #index = self.n_layer - i - 1

            if i == step:
                out = self.from_rgb[i](input)

            if i == 0:
                out = minibatchStat(out,split)
