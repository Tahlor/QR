...
from .style_gan import StyledConvBlock, EqualConv2d, PixelNorm, EqualLinear, ConvBlock

class GrowGenU(nn.Module):
    def __init__(self, code_dim, fused=True, n_mlp=6):
        ...
        self.progression = nn.ModuleList(
            [
                ...
                StyledConvBlock(512+256, 512, 3, 1, upsample=True),  # 8
                StyledConvBlock(512+256, 256, 3, 1, upsample=True),  # 16
                StyledConvBlock(256+128, 128, 3, 1, upsample=True),  # 32
                StyledConvBlock(128+64, 64, 3, 1, upsample=True),  # 64
                StyledConvBlock(64+32, 32, 3, 1, upsample=True, fused=fused),  # 128
                StyledConvBlock(32+16, 16, 3, 1, upsample=True, fused=fused),  # 256
    ...
    def forward(self, qr_image, style=None, step=0, alpha=-1, mixing_range=(-1, -1)):
        ...
        for i, (conv, to_rgb) in enumerate(zip(self.progression, self.to_rgb)):
            ...
            if i>0:
                out = torch.cat((out,prev_xs[i-1]),dim=1)   
