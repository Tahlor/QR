...
class QRGrowGenTrainer(BaseTrainer):
    """
    Trainer class
    ...
    def _train_iteration(self, iteration):
        ...
        """
        into_phase = iteration%self.phase
        self.alpha = min(1, (1 / self.phase) * (into_phase + 1))
        self.step = min(self.max_step,self.init_step+iteration//self.phase)
        if self.resolution == self.init_size  or self.step==self.max_step:
            self.alpha=1
        resolution = 4 * 2 ** self.step
        if resolution != self.resolution:
            self.resolution=resolution
            adjust_lr(self.optimizer, self.lr_schedule.get(resolution, 0.001))
            adjust_lr(self.optimizer_discriminator, self.lr_schedule.get(resolution, 0.001))
            print('New resolution: {}x{}'.format(resolution,resolution))
    ...
    def run(self,instance,lesson,get=[]):
        ...
        if 'gen' in lesson or 'disc' in lesson or 'gen' in get:
            gen_image = self.model(
                    qr_image,
                    style='mixing' if 'eval' not in lesson and 'valid' not in lesson else None,
                    step=self.step,
                    alpha=self.alpha
                    )
        ...
        if 'sample-disc' in lesson or 'disc' in lesson:
            ...
            if real.size(2) != self.resolution:
                real = F.interpolate(real,(self.resolution,self.resolution),mode='bilinear')
        ...
        if 'disc' in lesson or 'auto-disc' in lesson or 'sample-disc' in lesson:
            ...
            discriminator_pred = self.model.discriminator(torch.cat((real,fake),dim=0).detach(),True,step=self.step,alpha=self.alpha)
            if self.WGAN:
                #Improved W-GAN
        ...
        if ('gen' in lesson or 'auto-gen' in lesson) and 'eval' not in lesson:
            ...
            gen_pred = self.model.discriminator(fake,False,step=self.step,alpha=self.alpha)
            gen_loss=0
            predicted_disc=[]
            if self.DCGAN:
                for gp in gen_pred:
