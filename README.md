...
[]{data-label="fig:coke"}](images/other_features/DOUBLE.png)
...
image.[]{data-label="fig:generator"}](images/qr_gan_offset_arch.png)

The generator in QR-GAN accepts a 256×256 image QR code and
outputs a new 256×256 image, which should decode with the same
...
blocks and up-sampling, starting from a learned constant 4×4
feature tensor and ending at the desired resolution. In our case, we
up-sample to 256×256. We modify the architecture to have a
down-sampling CNN branch, which reduces the dimensions down from
256×256 to 4×4. The input QR code (image) is passed into
...
still includes the starting 4×4 feature tensor that StyleGAN2
...
shifts.[]{data-label="fig:bad_gen"}](images/bad_gen.png)
...
at the center pixel of each data cell, a value of 0.5 in the 3×3
area around this and 0.25 in the 5×5 in the area around this,
...
training.[]{data-label="fig:genloss"}](images/genloss.png)

![Masked QR reconstruction loss during
training.[]{data-label="fig:pixloss"}](images/pixloss.png)
...
post-processing.[]{data-label="fig:lowvalid"}](images/other_features/THE_TRIPLE.png)

Results
=======

We test our method on several datasets. All images were resized to
256×256. We used QR code version 1 (22×22 cells) with the
...
dataset.[]{data-label="fig:art"}](images/art_data.png)

![Generated images from model trained on abstract art
dataset.[]{data-label="fig:art_gen"}](images/art_gen.png)
...
images.[]{data-label="fig:texture_gen"}](images/texture_data.png)

![Generated images from model trained on texture
images.[]{data-label="fig:texture_gen"}](images/texture_gen.png)
...
dataset.[]{data-label="fig:water_gen"}](images/water_data.png)

![Generated images from model trained on satellite images. The model
only captures color information from the
dataset.[]{data-label="fig:water_gen"}](images/water_gen.png)
...
artifacts.[]{data-label="fig:fabric_gen"}](images/fabric_data.png)

![Generated images from model trained on African fabric images. Because
the fabric images are low resolution, the generated images contain pixel
artifacts.[]{data-label="fig:fabric_gen"}](images/fabric_gen.png)
...
in this dataset had low resolution (64×64) and had to be
up-sampled to 256×256, the generator learned pixel artifacts.

LSUN bedrooms
-------------

![Generated images from model trained on bedroom images from the LSUN
dataset.[]{data-label="fig:bedroom_gen"}](images/bedrooms_data.png)

![Generated images from model trained on bedroom images from the LSUN
dataset.[]{data-label="fig:bedroom_gen"}](images/bedrooms_gen.png)
